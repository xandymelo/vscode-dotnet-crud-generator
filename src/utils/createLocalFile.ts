...
import { CreateFile, globalFileNames } from '../constants';

export async function createFiles(filesToCreate: CreateFile[], currentPath: string, modelContent: string) {
    for (const file of filesToCreate) {
        const fileName = file.type === globalFileNames.generateModels ? file.name + "Model.cs" : 
            file.type === globalFileNames.generateController ? file.name + "Controller.cs" :
